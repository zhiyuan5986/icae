...
with torch.no_grad():
    with open("ft_inference_llama.out", "w") as f:
        ...
        for line in tqdm(lines):
            ...
            prompt_right_ids = [model.ft_token_id] + tokenized_prompt['input_ids'] + [model.ft_token_id]
            ...
            decoder_input_embeddings = torch.cat((memory_slots.unsqueeze(0), prompt_right_embs), dim=1)
            ...
            for i in range(max_out_length):
                ...
                if next_token_id.item() == 1 or next_token_id.item() >= model.tokenizer.vocab_size:   # eos
                    break
